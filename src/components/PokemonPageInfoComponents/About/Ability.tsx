...
import { AbilitiesEntity } from "../../../types/pokemon";
import { type IAbility } from "../../../types/abilityDetails";

const Ability = ({ ability }: { ability: AbilitiesEntity }) => {
  const [showAbilityPopup, setShowAbilityPopup] = React.useState(false);
  const [isScrollable, setIsScrollable] = React.useState(false);

  const abilityEffectToDisplay = (abilityDetails: IAbility) => {
  ...
  const abilityPopup = React.useRef<HTMLDivElement>(null);

  React.useLayoutEffect(() => {
    if (
      abilityPopup?.current &&
